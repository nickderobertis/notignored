#!/usr/bin/env node
// Launcher for the `notignored` command installed from the `notignored-cli` npm
// package.
//
...
// inside a per-platform package (`notignored-cli-<platform>-<arch>`) declared in
...
const PACKAGES = {
  "linux-x64": "notignored-cli-linux-x64",
  "linux-arm64": "notignored-cli-linux-arm64",
  "darwin-x64": "notignored-cli-darwin-x64",
  "darwin-arm64": "notignored-cli-darwin-arm64",
  "win32-x64": "notignored-cli-win32-x64",
};

// Every failure here is a failed install, so say what to do about it rather than
// only what went wrong.
const OTHER_INSTALLS =
  "Install another way instead: 'pip install notignored-cli', " +
  "'cargo install --git https://github.com/nickderobertis/notignored --locked', " +
  "or the install script at https://github.com/nickderobertis/notignored#install";

function fail(message) {
  process.stderr.write(`notignored: ${message}\n`);
  process.exit(1);
}
...
if (result.error) {
  fail(`failed to launch the notignored binary: ${result.error.message}`);
}

// Re-raise a terminating signal so callers observe the true cause; otherwise
// propagate the child's exit code verbatim — `--fail-if-found` exits 1 and CI
