...
def test_diff_reports_only_the_suppressions_this_change_added(
    notignored_binary: Path, git_repo: Path
) -> None:
    ...
    whole_tree = scan(cwd=git_repo, binary=notignored_binary)
    changed = scan(diff=True, cwd=git_repo, binary=notignored_binary)
...
def test_diff_against_a_base_branch_reports_what_the_branch_added(
    notignored_binary: Path, git_repo: Path
) -> None:
    ...
    changed = scan(diff=True, diff_base="main", cwd=git_repo, binary=notignored_binary)

    assert [directive.path for directive in changed.ignores] == ["feature.py"]


def test_diff_finds_nothing_when_the_change_added_no_suppression(
    notignored_binary: Path, git_repo: Path
) -> None:
    (git_repo / "app.py").write_text(
    ...
    assert scan(diff=True, cwd=git_repo, binary=notignored_binary).ignores == ()


def test_the_async_form_diffs_the_same_way(notignored_binary: Path, git_repo: Path) -> None:
    (git_repo / "app.py").write_text(
        (git_repo / "app.py").read_text(encoding="utf-8") + ADDED, encoding="utf-8"
    )

    assert asyncio.run(ascan(diff=True, cwd=git_repo, binary=notignored_binary)) == scan(
        diff=True, cwd=git_repo, binary=notignored_binary
    )


def test_a_base_with_nothing_to_compare_is_rejected_before_a_process_starts(
    notignored_binary: Path, git_repo: Path
) -> None:
    """The CLI rejects `--diff-base` without `--diff`; the SDK says so first."""
    with pytest.raises(ValueError, match="diff=True"):
        scan(diff_base="main", cwd=git_repo, binary=notignored_binary)

    with pytest.raises(ValueError, match="diff=True"):
        asyncio.run(ascan(diff_base="main", cwd=git_repo, binary=notignored_binary))
