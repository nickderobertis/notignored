...
//!
//! `#[allow]` inside `#[cfg_attr(test, allow(…))]` is **not** reported: it
//! applies only under that configuration, and claiming an unconditional
//! suppression would over-report.
...
mod tests {
    ...
    fn attributes_that_are_not_suppressions_are_skipped() {
        ...
        assert!(parse("#[allowance(dead_code)]\nfn f() {}\n").is_empty());
        assert!(parse("#[doc = \"allow(dead_code)\"]\nfn f() {}\n").is_empty());
        assert!(parse("#[cfg_attr(test, allow(dead_code))]\nfn f() {}\n").is_empty());
        assert!(parse("#[allow()]\nfn f() {}\n").is_empty());
        assert!(parse("#[expect(reason = \"nothing named\")]\nfn f() {}\n").is_empty());
        assert!(parse("#[allow\nfn f() {}\n").is_empty());
    }
