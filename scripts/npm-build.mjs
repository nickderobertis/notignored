#!/usr/bin/env node
// Build the npm packages that distribute the prebuilt notignored binary — the
...
//
//   notignored-cli                 launcher package (npm/notignored, committed)
//     bin/notignored.js            resolves + execs the platform binary
//     optionalDependencies:        one per Rust target in release.yml's matrix
//       notignored-cli-linux-x64
//       notignored-cli-linux-arm64
//       notignored-cli-darwin-x64
//       notignored-cli-darwin-arm64
//       notignored-cli-win32-x64   each carries the matching prebuilt binary
//
...
// `npm install -g notignored-cli` is a seconds-fast binary install — the same
...
// the PACKAGES map in npm/notignored/bin/notignored.js and the
// optionalDependencies in npm/notignored/package.json.
const TARGETS = {
...
function buildPlatform(args) {
  ...
  const binName = facts.exe ? "notignored.exe" : "notignored";

  // Resolve the source binary with a `.exe` fallback: a bash caller may pass the
  // extensionless path (Git Bash's `test -x` matches notignored.exe
  ...
  writeJson(join(pkgDir, "package.json"), {
    name: pkgName,
    version,
    description: `Prebuilt notignored binary for ${facts.platform} ${facts.arch}.`,
    homepage: REPOSITORY,
    license: "MIT",
